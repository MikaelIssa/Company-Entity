...
public class DbService : IDbService
{
    ...
    bool>> expression) where TEntity : class, IEntity => await _db.Set<TEntity>().SingleOrDefaultAsync(expression);
    ...
    {
        var succeess = await _db.Set<TEntity>().AnyAsync(expression);
        return succeess;
    ...
    public async Task<bool> DeleteAsync<TEntity>(int id) where TEntity : class, IEntity  //7.17
    {
        try
        {
